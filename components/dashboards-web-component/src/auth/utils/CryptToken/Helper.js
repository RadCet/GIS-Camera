...
export default class EncryptHelper {
    ...
    static encodeKey2(rawString) {
        let e = EncryptHelper.encodeKey(rawString);
        let random = require("randomstring").generate(e.length);
        var output = "";
    ...
    static decodeKey2(rawString) {
        ...
        return EncryptHelper.decodeKey(output);
    }

    static encodeKey3(rawString) {
        let e = Buffer.from(rawString, "utf8").toString('base64');
        let random = require("randomstring").generate(e.length);
        var output = "";
        for (let i = 0; i < e.length; i++) {
            output += e[i];
    ...
    static decodeKey3(tokenStr) {
        ///old///
        var base64Token = "";
        for(var i = 0; i < tokenStr.length; i++) {
            if(i%2==0) {
                base64Token += tokenStr[i];
            }
        }
        let username = "admincamera";
        let password = "";
        try {
            // password = atob(base64Token);
            password = Buffer.from(base64Token, 'base64').toString("utf8");
        }
        catch(error) {
        }
