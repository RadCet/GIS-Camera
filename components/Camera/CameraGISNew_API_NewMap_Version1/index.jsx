...
export default class GeoChartCamera extends Widget {
  ...
  componentDidMount() {
    ...
    console.log("________________v________________" + 16);
      ...
      view.on("pointer-move", function(event) {
        view.hitTest(event).then(function(response) {
          ...
          response.results.filter(function(result) {
            ...
            let clickedMarker = dataCamera[graphicMarker.layer.layerId].filter(
  ...
  updateCameraDataHandler(datas) {
    ...
    let nodeHandler = (monitor, node, parentNode) => {
      ...
      if (monitor.cameraData != null) {
        monitor.cameraData.values.push(node.value);
        return monitor.cameraData;
      }
      const camera = {
        id: monitor.Id,
        vmsCamId: monitor.Id,
  ...
  showLiveCamOnMobile(vmsCamId, clusterDataID = null) {
    ...
    if (liveCameraData) {
      ...
      console.log("liveCamSrc:" + liveCamSrc);
      this.setState({
  ...
  addMarkerToMap() {
    ...
    displayCamera = displayCamera.filter(item => {
      return (
        this.state.currentFilter == StatisData.ALL ||
        (this.state.currentFilter == StatisData.UNBROKEN &&
          (item.level === 1 || item.level === 2)) ||
        (this.state.currentFilter == StatisData.ACTIVE && item.level === 1) ||
        (this.state.currentFilter == StatisData.UNACTIVE && item.level === 2) ||
        (this.state.currentFilter == StatisData.BROKEN && item.level === 0) ||
        (this.state.currentFilter == StatisData.AI &&
          (item.ailevel === 3 || item.ailevel === 4 || item.ailevel === 5)) ||
        (this.state.currentFilter == StatisData.EVENT5P &&
          item.ailevel === 4) ||
        (this.state.currentFilter == StatisData.EVENT24H &&
          (item.ailevel === 5 || item.ailevel === 4)) ||
        (this.state.currentFilter == StatisData.SOCIAL && item.glevel === 6)
      );
    });
  ...
  onChangeLayer(layer) {
    if (layer && layer.length > 0) {
      const cameraDataByLayer = this.getCameraDataByLayer(layer);
  ...
  removeFeatures(monumentLayer) {
    ...
    if (monumentLayer) {
      ...
      monumentLayer.queryFeatures().then(function(results) {
        ...
        // this.applyEditsToLayer(monumentLayer, deleteEdits);
      });
    }
  }
      ...
      <div style={style}>
        ...
        }
        {
          <div id={"leftSide"} style={showLeftMenu ? leftSide : leftSideHidden}>
            ...
            <div style={{ paddingTop: 15, height: 330 }}>
              <span
                style={{
                  fontSize: "17px",
                  fontWeight: "bold",
