export class VMS {
  static check(vms) {
    if (!vms) return false;
    if (typeof vms !== "object") return false;
    return true;
  }
  ...
  same(vms) {
    ...
    const { Hostname, Port, Protocol } = vms;
    if (Protocol != null && Port != null && Hostname != null) {
      return (
        this.Port == Port &&
        this.Hostname == Hostname &&
        this.Protocol == Protocol
  ...
  update(vms) {
    ...
    if (this.same(vms)) {
      if (vms.auth != null && vms.auth.access_token != null)
        this.auth = vms.auth;
...
export class VMSManager {
  ...
  updateDefaultData(default_target = null) {
    ...
    this.default_id =
      this.default_target == null ? null : this.default_target.getID();
...
export class VMSController {
  constructor(vms, config) {
    ...
    this.vms_update_token_api = this.updateAPIURL(
      vms_update_token_api_template
    );
    ...
    this.vms_monitors_groups_api = this.updateAPIURL(
      vms_monitors_groups_api_template
    );
    this.vms_users_groups_api = this.updateAPIURL(
      vms_users_groups_api_template
    );
    this.vms_get_tree_data_api = this.updateAPIURL(
      vms_get_tree_data_api_template
    );

    this.current_token =
      this.vms.auth == null ? null : this.vms.auth.access_token;
  }
  ...
  callAPIWithTemplate(
    ...
    return this.callAPI(
      this.updateAPIURL(apiUrlTemplate),
      param,
      method,
      responseMapHandler
    );
        ...
        if (status === 401 || status === 403) {
          ...
          if (
            ...
            if (
              ...
              this.current_token = null;
              return this.updateToken(true).then(token =>
                this.callAPI(apiUrl, params)
              );
            }
          }
        }
