...
class CameraVMSController {
  constructor(
    ...
    const {
      ...
      vms_domain,
      vms_port,
      vms_username_default,
      vms_password_default
    } = this.configs;
    const {
    ...
    const {
      ...
      vms_groups_api_template,
      vms_monitors_groups_api_template,
      vms_users_groups_api_template,
      vms_turn_on_camera_api_template,
      vms_get_live_camera_api_template,
      vms_get_tree_data_api_template,
    ...
    } = this.applyMultipleVMS ? this.vms_multi_cluster_config : this.configs;
    ...
    this.vms_update_token_api = vms_update_token_api_template
      .replace("{vms_protocol}", vms_protocol)
      .replace("{vms_domain}", vms_domain)
      .replace("{vms_port}", vms_port);
    this.vms_monitors_api = vms_monitors_api_template
      .replace("{vms_protocol}", vms_protocol)
      .replace("{vms_domain}", vms_domain)
      .replace("{vms_port}", vms_port);
    this.vms_groups_api = vms_groups_api_template
      .replace("{vms_protocol}", vms_protocol)
      .replace("{vms_domain}", vms_domain)
      .replace("{vms_port}", vms_port);
    this.vms_monitors_groups_api = vms_monitors_groups_api_template
      .replace("{vms_protocol}", vms_protocol)
      .replace("{vms_domain}", vms_domain)
      .replace("{vms_port}", vms_port);
    this.vms_users_groups_api = vms_users_groups_api_template
      .replace("{vms_protocol}", vms_protocol)
      .replace("{vms_domain}", vms_domain)
      .replace("{vms_port}", vms_port);
    this.vms_get_targets_api = vms_get_targets_api_template
      .replace("{vms_protocol}", vms_protocol)
      .replace("{vms_domain}", vms_domain)
      .replace("{vms_port}", vms_port);
    this.vms_get_tree_data_api = vms_get_tree_data_api_template
      .replace("{vms_protocol}", vms_protocol)
      .replace("{vms_domain}", vms_domain)
      .replace("{vms_port}", vms_port);
    this.vms_get_live_camera_api = vms_get_live_camera_api_template
      .replace("{vms_protocol}", vms_protocol)
      .replace("{vms_domain}", vms_domain)
      .replace("{vms_port}", vms_port);
    this.vms_get_define_error_api = vms_get_define_error_api_template
      .replace("{vms_protocol}", vms_protocol)
      .replace("{vms_domain}", vms_domain)
      .replace("{vms_port}", vms_port);
  ...
  getListVMS() {
    return this.callAPI(this.vms_get_targets_api, {}).then(multi_cluster => {
      return multi_cluster == null
        ? this.vmsManager.getListVMS()
        : multi_cluster
            .map(item => this.vmsManager.updateVMS(item.target))
  ...
  updateToken(isForce = false) {
    ...
    let { current_vms } = this.state;
    if (current_vms == null)
      return this.loadVMS().then(vms => this.updateToken());
    if (!isForce && this.current_token != null)
      return Promise.resolve(this.current_token);
    const { username, password } = current_vms;
    const vms_access_token_api =
      current_vms.auth == null ? null : current_vms.auth.access_token;
    const vms_refresh_token_api =
      current_vms.auth == null ? null : current_vms.auth.refresh_token;
    let isNeedRequestLogin =
      vms_access_token_api == null || vms_refresh_token_api == null; //and expire
    let apiUrl = isNeedRequestLogin
      ? this.vms_login_api
      : this.vms_update_token_api;
    let params = isNeedRequestLogin
      ? { user: username, pass: password }
      : { token: vms_refresh_token_api };
    ...
    return axios
      ...
      .then(resp => {
        if (!!resp.data && !!resp.data.access_token) {
          //const {access_token, refresh_token, access_token_expires, refresh_token_expires} = resp.data;
    ...
    return axios({
            ...
            if (this.isConnectDirectToCluster) {
              ...
              if (Array.isArray(response)) {
                if (response.length > 0 && response[0].target == null) {
                  response = [
                    { target: this.getDefaultTarget(), data: response }
                  ];
                }
              } else {
                response = [
      ...
      .catch(error => {
        ...
        if (status === 401 || status === 403) {
          ...
          if (
            current_vms.auth != null &&
            current_vms.auth.token_time_update != null
          ) {
            if (
              new Date().getTime() -
                new Date(current_vms.auth.token_time_update).getTime() >
              current_vms.auth.access_token_expires * 1000
            ) {
              this.current_token = null;
              return this.updateToken().then(token =>
                this.callAPI(
  ...
  loadCameras() {
    ...
    const normalDataHandler = (index, value, name, target) => {
      ...
      if (returnValue == null) {
        console.error(
          `${methodTag}:Input invalid:${name}:${value}:[${
            target == null ? null : target.Hostname
          }]`
        );
        ...
        if (this.applyMultipleVMS) {
          ...
          let listTargets =
            datas == null || datas.length === 0
              ...
              : datas
                  .filter(
                    item => !(this.ignoreDefaultTargetData && item.target.root)
                  )
                  .map(item => this.vmsManager.updateVMS(item.target))
          ...
          const dataCombineHandler = dataItem =>
            datasName.reduce((output, _name, index) => {
              ...
              output[_name] = normalDataHandler(
                ...
                data,
                _name,
                this.vmsManager.getDefaultTarget()
              );
              return output;
            }, {});
    ...
    const tasks = [
      this.loadMonitors(),
      this.loadGroups(),
      this.loadMonitorsGroups(),
      this.loadUserGroupIDs()
    ];
    return Promise.all(tasks)
      .then(datas => {
        if (
          datas == null ||
          datas.length < tasks.length ||
          datas.filter(item => !Array.isArray(item)).length > 0
        ) {
  ...
  turnOnCameraToVMS(vmsID, vmsCameraID) {
    ...
    return this.callAPIToVMS(
      vmsID,
      this.vms_turn_on_camera_api_template.replace("{id}", vmsCameraID),
      {},
      null,
      "put"
    );
  ...
  getInformationConditionCamera(vmsID, idCamera) {
    return this.callAPIToVMS(
      vmsID,
      this.vms_get_information_report_camera_template.replace(
        "{idCamera}",
        idCamera
      ),
      {},
  ...
  getHistoryReportCamera(vmsID, idCamera, paramData) {
    return this.callAPIToVMS(
      vmsID,
      this.vms_get_history_report_camera_template.replace(
        "{idCamera}",
        idCamera
      ),
      paramData,
  ...
  submitCameraConditionForm(vmsID, idCamera, paramData) {
    return this.callAPIToVMS(
      vmsID,
      this.vms_submit_form_report_camera_template.replace(
        "{idCamera}",
        idCamera
      ),
      paramData,
  ...
  getDataReportCamera(vmsID) {
    return this.callAPIToVMS(
      vmsID,
      this.vms_get_data_report_camera_template,
      {},
      null,
      "get"
    );
