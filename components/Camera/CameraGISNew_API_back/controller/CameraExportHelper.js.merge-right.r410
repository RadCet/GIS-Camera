export default class CameraExportHelper {
    ...
    static exportData(currentLayer, cameraData, treeData) {
        ...
        CameraExportHelper.convertToCSV3({ children: treeData }, currentLayer, cameraData);
        // this.convertToCSV3({ children: this.state.treeData });
        // let result = { value: '' };
        // this.convertToCSV2(result, { children: this.state.treeData });
        // const date = new Date();
        // let fileTitle = 'Thong_ke_Camera_' + date.getFullYear() + '_' + (date.getMonth() + 1) + '_'
        //     + date.getDate() + '_' + date.getHours() + '_' + date.getMinutes();
        // let csv = 'Lớp cam, Số cam hoạt động, Số cam không hoạt động\n';

        // const { cameraData, currentLayer } = this.state;
        // const cameraDataByLayer = cameraData.filter((item) => {
        //     const itemKey = item.layer + '.' + item.level1 + '.' + item.level2;
        //     return (typeof (currentLayer) == 'undefined') || itemKey.startsWith(currentLayer);
        // });
        ...
        // a.setAttribute('download', exportedFilenmae);
        // a.click();
    }
    ...
    static convertToCSV2(result, treeData, currentLayer) {
        ...
        if (treeData.name && treeData.value && (typeof (currentLayer) == 'undefined' || treeData.value.startsWith(currentLayer))) {
            if (!treeData.level) {
                result.value += '\n';
            }
    ...
    static convertToCSV3(treeData, currentLayer, cameraData) {
        ...
        let cameraDataByLayer = cameraData.filter((item) => {
            return (typeof (currentLayer) == 'undefined') || item.values.find(item => item.startsWith(currentLayer)) != null;
        });
        ...
        const _camBroken = cameraDataByLayer.filter((item) => item.level === 0).length;

        const _camActive = cameraDataByLayer.filter((item) => item.level === 1).length;
        const _camUnactive = cameraDataByLayer.filter((item) => item.level === 2).length;
        const _camUnbroken = _camActive + _camUnactive;

        const _camAINoEvent = cameraDataByLayer.filter((item) => item.ailevel === 3).length;
        const _camAI5pEvent = cameraDataByLayer.filter((item) => item.ailevel === 4).length;
        const _camAI24hEvent = cameraDataByLayer.filter((item) => item.ailevel === 5).length;
        ...
        const merge = [
            ...
            { s: { r: 3, c: 0 }, e: { r: 3, c: 2 } }, { s: { r: 6, c: 0 }, e: { r: 6, c: 1 } },
            // { s: { r: 6, c: 2 }, e: { r: 6, c: 3 } }
        ];
        ...
        const date = new Date();
        wsOverView['A1'].v = "                                                                         Cộng hòa xã hội chủ nghĩa Việt Nam";
        wsOverView['A2'].v = "                                                                              Độc lập - Tự do - Hạnh phúc";
        wsOverView['A4'].v = "                          BÁO CÁO TÌNH HÌNH CAMERA ĐÃ TÍCH HỢP LÊN TRUNG TÂM ĐIỀU HÀNH IOC TỈNH QUẢNG NINH";
        wsOverView['B6'].v = "          Ngày: " + date.getDate() + "/" + (date.getMonth() + 1) + "/" + date.getFullYear();
        wsOverView['C6'].v = "          Thời gian xuất báo cáo: " + date.getHours() + ":" + date.getMinutes();
        ...
        treeData.children.filter((category) => {
            return (category.name === "Tiếp Dân" || category.name === "Hành Chính Công");
        }).map((type) => {
        ...
        treeData.children.map((categoty) => {
            ...
            wsOverView['C' + indexRow].v = categoty.allLength;
            indexRow++;
        })
        ...
        listNameCategory.map((nameCategory) => {
            ...
            wsCategory['A1'].v = "                                           BÁO CÁO TÌNH HÌNH CAMERA " + nameCategory.toUpperCase();
            ...
            const cameraCategory = treeData.children.filter((item) => item.name === nameCategory);

            // console.log(cameraCategory[0]);
            ...
            if (nameCategory === "UBND Tỉnh") {
                ...
                wsCategory['C' + indexRowCategory].v = cameraCategory[0].liveLength;
                wsCategory['D' + indexRowCategory].v = cameraCategory[0].allLength - cameraCategory[0].liveLength;
                indexRowCategory++;
            } else {
                ...
                cameraCategory[0].children.map((district) => {
                    ...
                    wsCategory['C' + indexRowCategory].v = district.liveLength;
                    wsCategory['D' + indexRowCategory].v = district.allLength - district.liveLength;
                    indexRowCategory++;
                    district.children.map((ward) => {
                        wsCategory['A' + indexRowCategory].v = indexSTTCategory;
                        indexSTTCategory++;
                        wsCategory['B' + indexRowCategory].v = "      " + ward.name;
                        wsCategory['C' + indexRowCategory].v = ward.liveLength;
                        wsCategory['D' + indexRowCategory].v = ward.allLength - ward.liveLength;
                        indexRowCategory++;
                    })
                })
            }
