...
const cameraIcons = [cameraIcon0, cameraIcon1, cameraIcon2, cameraIcon3, cameraIcon4, cameraIcon5, cameraIcon6];
...
export default class GeoChartCamera extends Widget {
    ...
    componentDidMount() {
        ...
        super.getWidgetConfiguration(widgetID).then((message) => {
            ...
            this.latCenter = message.data.configs.providerConfig.mapConfig.configs.latCenter;
            this.lngCenter = message.data.configs.providerConfig.mapConfig.configs.lngCenter;
            this.constlatCenter = message.data.configs.providerConfig.mapConfig.configs.latCenter;
            this.constlngCenter = message.data.configs.providerConfig.mapConfig.configs.lngCenter;
            this.zoomDefault = message.data.configs.providerConfig.mapConfig.configs.zoomDefault;
            this.northeastLat = message.data.configs.providerConfig.mapConfig.configs.northeastLat;
            this.northeastLng = message.data.configs.providerConfig.mapConfig.configs.northeastLng;
            this.southwestLat = message.data.configs.providerConfig.mapConfig.configs.southwestLat;
            this.southwestLng = message.data.configs.providerConfig.mapConfig.configs.southwestLng;
            this.defaultSearch = message.data.configs.providerConfig.mapConfig.configs.defaultSearch ?
                message.data.configs.providerConfig.mapConfig.configs.defaultSearch : '';

            this.minZoom = message.data.configs.providerConfig.mapConfig.configs.minZoom;
            this.maxZoom = message.data.configs.providerConfig.mapConfig.configs.maxZoom;
            this.autoZoomForSelectedGroup = message.data.configs.providerConfig.mapConfig.configs.autoZoomForSelectedGroup;
            this.zoomScaleValues = message.data.configs.providerConfig.mapConfig.configs.zoomScaleValues;
            if (this.minZoom == null) this.minZoom = 2;
            if (this.maxZoom == null) this.maxZoom = 19;
            if (this.zoomScaleValues == null || !Array.isArray(this.zoomScaleValues)) this.zoomScaleValues = [];
            this.zoomScaleValues = this.zoomScaleValues.filter(item => !isNaN(item) && item >= 0).sort((a,b) => a-b);
            this.zoomScaleWidthValues = message.data.configs.providerConfig.mapConfig.configs.zoomScaleWidthValues;
            this.zoomScaleHeightValues = message.data.configs.providerConfig.mapConfig.configs.zoomScaleHeightValues;
            if (this.zoomScaleWidthValues == null || !Array.isArray(this.zoomScaleWidthValues) || this.zoomScaleWidthValues.length === 0) {
                this.zoomScaleWidthValues = this.zoomScaleValues;
            } else {
                this.zoomScaleWidthValues = this.zoomScaleWidthValues.filter(item => !isNaN(item) && item >= 0).sort((a,b) => a-b);
            }
            if (this.zoomScaleHeightValues == null || !Array.isArray(this.zoomScaleHeightValues) || this.zoomScaleHeightValues.length === 0) {
                this.zoomScaleHeightValues = this.zoomScaleValues;
            } else {
                this.zoomScaleHeightValues = this.zoomScaleHeightValues.filter(item => !isNaN(item) && item >= 0).sort((a,b) => a-b);
            }
            ...
            const {monitorsFieldSearch, va_support, socialization_support,
                showSocializationInNewTab, showVMSCountIgnoreDefault,
                showVMSCountInList, showVMSCountInListInTop, showVMSCountInListInBottom,
                showElapTime, showReload, hiddenExportInMobile
            } = this.apiConfig;
            this.va_support = va_support == null ? true : va_support;
            this.showSocializationInNewTab = showSocializationInNewTab == null ? true : showSocializationInNewTab;
            this.showVMSCountInList = showVMSCountInList == null ? true : showVMSCountInList;
            this.showVMSCountInListInTop = showVMSCountInListInTop == null ? true : showVMSCountInListInTop;
            this.showVMSCountInListInBottom = showVMSCountInListInBottom == null ? false : showVMSCountInListInBottom;
            this.socialization_support = socialization_support == null ? true : socialization_support;
            this.showVMSCountIgnoreDefault = showVMSCountIgnoreDefault == null ? false : showVMSCountIgnoreDefault;
            this.monitorsFieldSearch = monitorsFieldSearch == null ? ["name"] : monitorsFieldSearch;
            this.showElapTime = showElapTime == null ? false : showElapTime;
            this.showReload = showReload == null ? true : showReload;
            this.hiddenExportInMobile = hiddenExportInMobile == null ? true : hiddenExportInMobile;
            this.va_event_view = this.apiConfig.va_event_view;
            this.va_event_type_map = this.apiConfig.va_event_type_map;
            this.cameraVMSController = new CameraVMSController(this.apiConfig, this.updateCameraDataHandler, this.newTokenUpdateHandler);
            this.cameraVAController = new CameraVAController(this.apiConfig, this.cameraVMSController.vmsManager, this.updateAICameraHandler, this.updateAIEventHandler);
            ...
            this.timeElapseInterval = this.showElapTime ? setInterval(() => {
                let now = new Date();
                let elapse = (this.state.timeDataUpdate == null ? 0 : now.getTime() - this.state.timeDataUpdate.getTime()) / 1000;
                let hour = Math.floor(elapse / 60);
                let second = Math.floor(elapse - 60 * hour);
                let timeElapse = `${hour < 10 ? "0" : ""}${hour}:${second < 10 ? "0" : ""}${second}`;
                this.setState({
    ...
    updateAICameraHandler(aicameras, clusterDataID = null) {
        ...
        let cameraDataInCluster = cameraData.filter(camera => this.filterByClusterIDHandler(camera, clusterDataID));
        aicameras.forEach(aicamera => {
            let camera = cameraDataInCluster.find(camera => camera.vmsCamId == aicamera.vmsCameraId);
            if (camera != null) {
    ...
    updateAIEventHandler(aievents, clusterDataID = null) {
        const {lastestEvent, newestAIcamerasHaveEvent, allAIcamerasHaveEvent} = aievents;
        ...
        lastestEvent.forEach(event => {
            ...
            if (camera == null) {
                camera = cameraDataInCluster.find(camera => camera.vmsCamId == event.vmsCameraId);
                if (camera != null) {
            ...
            if (camera.events.length < 10) {//order.... save recently
    ...
    updateCameraDataHandler(datas) {
        const cameraData = [];
        let isFirst = this.state.cameraData == null || this.state.cameraData.length === 0;
        let minLat = null;
        let maxLat = null;
        let minLng = null;
        let maxLng = null;
        let nodeHandler = (monitor, node, parentNode) => {
            ...
            let level = monitor.Enabled == 1 ? ("Connected" === monitor.Status ? 1 : 0) : ("Connected" === monitor.LastStatusBKA ? 2 : 0);//"Connected" === monitor.Status ? monitor.Enabled == 1 ? 1 : 2 : 0;
            if (level > 0) {
            ...
            if (monitor.cameraData != null) {
                ...
                if (this.monitorsFieldSearch.indexOf("values") >= 0 || this.monitorsFieldSearch.indexOf("svalues") >= 0) {
        ...
        const {alwaysShowRootNode, ignoreGroupOnlyHaveOneChild, ignoreCameraNotInGroup, addCameraChildInTree, ignoreGroupInvalid} = this.viewConfig.treeConfig;
        let tree = CameraTreeHelper.updateTreeData(datas, ignoreGroupInvalid, ignoreCameraNotInGroup);
        let root = CameraTreeHelper.buildUITreeData(tree, null, addCameraChildInTree, nodeHandler);
        // Mock social camera.
        ...
        if (minLat != null && maxLat != null) this.latCenter = (minLat + maxLat) / 2;
        if (minLng != null && maxLng != null) this.lngCenter = (minLng + maxLng) / 2;
        ...
        console.log(`[lat(${minLat}-${maxLat}):lng(${minLng}-${maxLng})][center(${this.latCenter},${this.lngCenter})][south(${this.southwestLat},${this.southwestLng})][north(${this.northeastLat},${this.northeastLng})][delta(${deltaLat},${deltaLng}---${Math.max(deltaLat, deltaLng)}]})]`);
        ...
        if (this.va_support) {
            Promise.all([ this.cameraVAController.loadAICameras(), this.cameraVAController.loadEvents()]);
        }
    }

    getZoomBySize(width, height) {
        // TODO udate zoom with size, resolutive, screen size...
        if (width == null || width < 0 || height == null || height < 0) return this.zoomDefault;
        let delta = Math.max(width, height);
        let zoomWidth = (this.zoomScaleWidthValues == null || this.zoomScaleWidthValues.length === 0) ? this.zoomDefault : this.maxZoom + 1;
        ...
        let zoomHeight = (this.zoomScaleHeightValues == null || this.zoomScaleHeightValues.length === 0) ? this.zoomDefault : this.maxZoom + 1;
        ...
        console.log(`${this.zoomScaleWidthValues}|${this.zoomScaleHeightValues}|${this.zoomDefault}[${width},${height}]-->[${zoomWidth},${zoomHeight},${zoom}]`);
        return zoom
    }
    ...
    handleClickCamera(camera) {
        if (camera.level === 0) {
        ...
        } else {//if (camera.level == 1 || camera.level == 2 || camera.level == 3 || camera.level == 4 || camera.level == 5) {
            if (camera.glevel == 6) {
            ...
            } else if (this.state.isMobile || this.state.showPopupLiveCam) {// || camera.glevel == 6) {
                this.showLiveCamOnMobile(camera.vmsCamId, camera.clusterDataID);
            }
            else {
    ...
    addLiveCamera(vmsCamId, level, clusterDataID = null) {
        this.setState((prevState) => {
            return {
                liveCamera: prevState.liveCamera.find(item => item.vmsCamId == vmsCamId && this.filterByClusterIDHandler(item, clusterDataID)) != null
                    ? prevState.liveCamera
                    : prevState.liveCamera.length === 3
                        ? prevState.liveCamera.slice(1, prevState.liveCamera.length).concat({ vmsCamId, level, clusterDataID })
    ...
    showInfoWindow(camera) {
        ...
        const markerIcon = camera.ailevel != null ? cameraIcons[camera.ailevel] : cameraIcons[camera.level];
    ...
    showLiveCamOnMobile(vmsCamId, clusterDataID = null) {
        const liveCameraData = this.state.cameraData.find((cam) => cam.vmsCamId === vmsCamId && this.filterByClusterIDHandler(cam, clusterDataID));
        if (liveCameraData) {
            let liveCamSrc =  liveCameraData.Liveview;// resp.data.monitor.mjpeg;
            if (liveCameraData.glevel === 6) {
                liveCamSrc = `${liveCameraData.url}${liveCameraData.url.indexOf('?') > 0 ? '&' : '?'}`;
                if (liveCamSrc.indexOf("username=") < 0) {
                    liveCamSrc = `${liveCamSrc}${liveCamSrc.indexOf('?') > 0 ? '&' : '?'}username=${liveCameraData.username}`
                }
                if (liveCamSrc.indexOf("password=") < 0) {
                    liveCamSrc = `${liveCamSrc}${liveCamSrc.indexOf('?') > 0 ? '&' : '?'}password=${liveCameraData.password}`
                }
            }
            if (liveCameraData.level === 2 && liveCameraData.glevel !== 6) {
                this.cameraVMSController.turnOnCameraToVMS(liveCameraData.clusterDataID, liveCameraData.vmsCamId)
                    .then(resp => {
                    }).catch(error => {
                }).then(() => {
                    this.setState({
    ...
    addMarkerToMap() {
        ...
        displayCamera = displayCamera.filter((item) => {
            return this.filterByClusterIDHandler(item, currentClusterDataID) &&
            (currentFilter == StatisData.ALL ||
                (currentFilter == StatisData.UNBROKEN && (item.level === 1 || item.level === 2)) ||
                ...
                (currentFilter == StatisData.AI && (item.ailevel === 3 || item.ailevel === 4 || item.ailevel === 5)) ||
                (currentFilter == StatisData.EVENT5P && item.ailevel === 4) ||
                (currentFilter == StatisData.EVENT24H && (item.ailevel === 5 || item.ailevel === 4)) ||
        ...
        displayCamera.map(marker => {
            ...
            const markerIcon = marker.ailevel != null ? cameraIcons[marker.ailevel] :  marker.glevel != null ? cameraIcons[marker.glevel] : cameraIcons[marker.level];
            const vIcon = new vbd.Icon({
                url: markerIcon,
                size: new vbd.Size(32, 32)
            });
            ...
            vbd.event.addListener(vMarker, 'mouseover', function (param) {
                ...
                this.timer = setTimeout(function () {
                    ...
                    $('.aEvent').click(function (event) {
                        ...
                        let apiUrl = isSupportVideo ? that.cameraVAController.getEventVideoLink(eventID, clusterdataid) : that.cameraVAController.getEventImageLink(eventID, clusterdataid);
                        that.setState({
    ...
    handleFilter(newFilter, newClusterDataID) {
        ...
        this.setState({ currentFilter: currentFilter , currentClusterDataID: currentClusterDataID});
    }

    handleCatchCameraClick(eventFrom, title, event) {
        let isSupportVideo = browser() !== "safari";
        let apiUrl = isSupportVideo ? this.cameraVAController.getEventVideoLink(event.id) : this.cameraVAController.getEventImageLink(event.id);
        this.setState({
            videoEventSrc: apiUrl,
            videoEventTitle: title,
            videoEventDataType: isSupportVideo ? "video" : "image",
    ...
    resetMap() {
        if (incidentMap) {
            console.log(`resetMap:${this.latCenter}:${this.lngCenter}:${this.zoomDefault}`);
            const centerDefault = new vbd.LatLng(this.latCenter, this.lngCenter);
    ...
    focusMap(camera) {
        ...
        console.log(`focusMap:${this.latCenter}:${this.lngCenter}:${this.zoomDefault}`);
    ...
    handleRemoveLiveCam(camId, clusterDataID = null) {
        ...
        this.setState({
            liveCamera: liveCam.filter((item) => item.vmsCamId != camId && this.filterByClusterIDHandler(item, clusterDataID)),
        });
    };
    ...
    onChangeLayer(layer) {
        ...
        if (layer && layer.length > 0) {
            ...
            if (cameraDataByLayer.length > 0)
                this.focusMap(cameraDataByLayer);
        } else {
    ...
    render() {
        ...
        if (!cameraDataByLayer) {
            ...
            console.log(`cameraDataByLayer:${currentLayer}:${cameraDataByLayer.length}`);
        }
        const vbdStyle = (!isMobile && liveCamera.length > 0) ? { height: '75%', width: '100%' } : { height: '100%', width: '100%' };
        const leftSide = !isMobile
            ? { height: '100%', width: width * 0.3, minWidth: 285, float: 'left', borderLeft: '1px solid #13222a', paddingLeft: '10px', overflowY: 'auto' }
            : { height: width, width: '100%' };

        const rightSide = !isMobile
            ? { height: '100%', width: width * 0.7, maxWidth: this.props.glContainer.width - 285, float: 'right' }
            : { height: '80%', width: '100%' };
            ...
            <div style={style}>
                {
                    <div style={rightSide}>
                        ...
                        <div id={'vbdContainer'} style={vbdStyle} />

                        {
                            !this.state.isMobile && liveCamera.length > 0 &&
                            <div id={'liveCamera'} style={{ height: '25%', width: '100%', borderTop: '1px solid #081922' }}>
                                <ListCameraComponent
                    ...
                    <div style={leftSide}>
                        <div>
                            <div style={{ paddingTop: '10px' }}>
                                <TreeSelect
                                    showSearch
                                    style={{ width: '100%', position: 'unset', paddingRight: '10px' }}
                            ...
                            <hr style={{ borderTop: '1px solid', marginTop: '10px', marginBottom: '10px' }} />
                        </div>
                        <div style={{ paddingTop: 15}}>
                            <span style={{ fontSize: '17px', fontWeight: 'bold', marginLeft: '10px' }}>{this.showElapTime ? `TÌNH TRẠNG CAMERA (${timeElapse})` : `TÌNH TRẠNG CAMERA`} </span>
                            {
                                this.showReload &&
                                <Button
                        ...
                        </div>
                        {
                            (
                                ...
                                <div style={{ paddingTop: 35 }}>
                                    <span style={{ fontSize: '17px', fontWeight: 'bold', marginLeft: '10px' }}>SỰ KIỆN GẦN ĐÂY</span>
                                    <hr style={{ borderTop: '1px solid', marginTop: '11px', marginBottom: '10px' }} />
                                    <ListEventComponent
                                        eventDetectData={eventDetectData}
                                        cameraData={cameraData}
                ...
                <Modal
                    ...
                    width={(height + 60) * 1280 / 720 < width ? (height + 60) * 1280 / 720 : width}
