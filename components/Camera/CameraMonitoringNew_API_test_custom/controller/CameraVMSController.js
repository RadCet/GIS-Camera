...
class CameraVMSController {
    ...
    constructor(configs, updateCamerasHandler, newTokenUpdateHandler = null, persistentHandler = defaultPersistentHandler) {
        ...
        this.ignoreDefaultTargetData = this.vms_multi_cluster_config.ignoreDefaultTargetData === true;
        this.combineDefaultGroupTarget = this.vms_multi_cluster_config.combineDefaultGroupTarget === true;
        this.combineRootByTag = this.vms_multi_cluster_config.combineRootByTag === true;
    ...
    getListVMS() {
        return this.callAPI(this.vms_get_targets_api, {})
            .then(multi_cluster => {
                return multi_cluster == null ? this.vmsManager.getListVMS() :
                    multi_cluster.map(item => this.vmsManager.updateVMS(item.target)).filter(item => item != null);
            });
    }
        ...
        const normalDataHandler = (index, value, name, target) => {
            ...
            if ("monitors_groups" === name) {
                returnValue = Array.isArray(value) ?
                    value.flatMap(mg => [...new Set(mg.MonitorId)].map(m => {
            ...
            return this.loadTreeData().then(datas => {
                ...
                if (this.applyMultipleVMS) {
                    const targetMapHandler = target => {
                        let dataItem = datas.find(item => target.same(this.vmsManager.normalVMSData(item.target)));
                    ...
                    let listTargets = (datas == null || datas.length === 0) ? [] :
                        datas.filter(item => !(this.ignoreDefaultTargetData && item.target.root))
                            .map(item => this.vmsManager.updateVMS(item.target)).filter(item => item != null);
                    cameraDatas = listTargets.map(targetMapHandler);
                } else {
                    const dataCombineHandler = (dataItem) => datasName.reduce((output, _name, index) => {
            ...
            if (this.applyMultipleVMS) {
                const targetMapHandler = target => {
                    let data = datas.reduce((data, dataArray, index) => {
                        ...
                        let _itemData = dataArray.find(item => target.same(this.vmsManager.normalVMSData(item.target)));
                        data[_name] = normalDataHandler(index, _itemData == null ? null : _itemData.data, _name, target);
                        return data;
                    }, {});
                    return {target, data};
                };
                let listTargets = (datas.length === 0) ? [] :
                    datas[0].filter(item => !(this.ignoreDefaultTargetData && item.target.root))
                        .map(item => this.vmsManager.updateVMS(item.target)).filter(item => item != null);
                cameraDatas = listTargets.map(targetMapHandler);
            } else {
                const targetMapHandler = (dataArrays) => dataArrays.reduce((data, dataArray, index) => {
                    let _name = datasName[index];
